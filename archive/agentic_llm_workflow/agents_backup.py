...
from agentic_llm_workflow.enhanced_tools import (
    find_value_in_gnb_enhanced, execute_historical_sql_enhanced, 
    get_live_network_kpis, check_live_network_status,
    get_parameter_recommendations, validate_parameter_change_safety,
...
def monitoring_agent(state: State) -> State:
    """
    Enhanced 5G Network Monitoring Agent for LangGraph.
    --------------------------------------------------
    LIQUID ZIMBABWE ENHANCEMENTS:
    - Hybrid live network + simulation monitoring
    - Real-time KPI assessment from Huawei iMaster MAE
    - Liquid Zimbabwe specific KPIs (RACH Success Rate, IBLER, etc.)
    - Cassava Technologies professional logging
    ...
    print("\n\n🔍 Liquid Zimbabwe Monitoring Agent - Powered by Cassava Technologies")

    # Initialize LLM agent with enhanced tools
    llm = init_agent()
    system_prompt = '''You are an enhanced network monitoring agent for Liquid Zimbabwe's RAN optimization system, powered by Cassava Technologies. 
        Your task is to analyze live Huawei network performance and provide professional insights.
    ...
    # Enhanced toolset with live network capabilities
    enhanced_tools = [
        execute_xapp_sql, calc_weighted_average,  # Original tools
        get_live_network_kpis, check_live_network_status,  # New live network tools
        get_network_mode_status  # Hybrid mode detection
    ]
    llm_agent = create_react_agent(llm, tools=enhanced_tools, prompt=system_prompt)
    
    # Load static configurations from config.yaml
    config = yaml.safe_load(open('config.yaml', 'r'))
    monitoring_wait_time = config['monitoring_wait_time']

    # Check network operational mode
    mode_status = get_network_mode_status()
    yield f"📊 Network Mode: {mode_status.split()[0]} {mode_status.split()[1]}"
    
    # Get live network status if available
    live_status = check_live_network_status()
    if "✅" in live_status:
        yield f"🌐 {live_status}"
        # Get current live KPIs for context
        live_kpis = get_live_network_kpis()
        if "📊" in live_kpis:
            yield f"Current Network Health:\n{live_kpis}"

    ######################################################################################################################################################
    ## Enhanced Monitor Weighted Average Gain for p0_nominal:

    try:
        param = "p0_nominal"
        p0_nominal_current = state["vars_current"][param]
        yield f"🔍 Monitoring {param} = {p0_nominal_current} (Liquid Zimbabwe Enhanced): "
        weight1, weight2 = config[f'{param}_WA_weights']

        yield f"⏳ Collecting KPIs for {monitoring_wait_time} seconds (hybrid mode)..."
        time.sleep(monitoring_wait_time)

        update_value_in_db(state["vars_current"])
        yield "📈 Aggregating data from live network and historical sources..."

        # Enhanced SQL query prompt with live network context
        prompt = f''' Your task is to create a SQL query, execute it using the execute_xapp_sql tool EXACTLY ONCE, and return the resulting answer dataframe. \
            ...
            CONTEXT: You are analyzing the Liquid Zimbabwe network parameter P0_NominalPUSCH (p0_nominal).
            This parameter controls uplink power control and affects RACH success rates.
        ...
        yield f"⚖️ Weighted Average Gain observed: {weighted_avg_gain:.2f}"
        
        if float(weighted_avg_gain)<0:
            message = f"\n⚠️ LIQUID ZIMBABWE ALERT: Weighted average gain is negative for {param}. Recommend parameter optimization. Press Process Query button to reconfigure."
            yield message
            print("🔚 Exiting Enhanced Monitoring Agent - Escalating to Configuration Agent\n\n")
        ...
        error_message = f"❌ Enhanced Monitoring Agent encountered an error on {param} due to {str(e)}\n"
        print(error_message)
        yield error_message
        return
...
def config_agent(state: State) -> State:
    """
    Enhanced 5G Network Configuration Agent for LangGraph.
    -----------------------------------------------------
    LIQUID ZIMBABWE ENHANCEMENTS:
    - Hybrid live network + historical data analysis
    - Real-time parameter querying from Huawei iMaster MAE
    - Enhanced safety validation for production deployments
    - Liquid Zimbabwe specific parameter mapping
    - Cassava Technologies professional recommendations
    
    This agent analyzes and suggests configuration adjustments for the supported 5G parameters:
    - p0_nominal → P0_NominalPUSCH (Uplink Power Control)
    - dl_carrierBandwidth → ReferenceSignalPower_PDSCH 
    - ul_carrierBandwidth → ReferenceSignalPower_PUSCH
    - att_tx → A3EventOffset (Handover Trigger)
    - att_rx → T310Timer (Connection Failure Detection)

    For each parameter, it:
    - Interprets user queries to detect the target parameter
    - Queries live network for current values (with fallback)
    - Collects and aggregates historical KPIs using SQL queries
    - Calculates weighted average gain based on user-defined weights
    - Provides safety-validated recommendations with rollback plans
    """
    
    print("\n\n🎯 Liquid Zimbabwe Configuration Agent - Powered by Cassava Technologies")
    
    # Initialize LLM Agent based on the config.yaml parameters
    llm = init_agent()

    # Enhanced system prompt for Liquid Zimbabwe context
    system_prompt = '''You are an enhanced network configuration agent for Liquid Zimbabwe's RAN optimization system, powered by Cassava Technologies. 
        Your task is to analyze both live Huawei network data and historical performance to provide safe, data-driven parameter recommendations.
        You have access to live network APIs, historical databases, and safety validation tools.
        Always prioritize live network data and provide professional insights with safety considerations.
        Reply to questions concisely, and exactly in the format directed to you.'''
    
    # Enhanced toolset with live network capabilities
    enhanced_tools = [
        find_value_in_gnb_enhanced, execute_historical_sql_enhanced, calc_weighted_average,  # Enhanced existing tools
        get_parameter_recommendations, validate_parameter_change_safety,  # New safety tools
        get_network_mode_status  # Hybrid mode detection
    ]
    llm_agent = create_react_agent(llm, tools=enhanced_tools, prompt=system_prompt)
    ...
    # Enhanced parameter classification with Liquid Zimbabwe context
    prompt0 = f'''
    You will receive a USER QUERY and a list of PARAMETERS for Liquid Zimbabwe's network optimization. 
    Your task is to classify the query into the correct parameter category, following the RULES below. 
    Your response must include a clear step-by-step Reason, followed by the Parameter.

    LIQUID ZIMBABWE PARAMETERS:
    - p0_nominal (P0_NominalPUSCH - Controls uplink power, affects RACH success rate)
    - dl_carrierBandwidth (ReferenceSignalPower_PDSCH - Downlink reference signal power)
    - ul_carrierBandwidth (ReferenceSignalPower_PUSCH - Uplink reference signal power) 
    - att_tx (A3EventOffset - Handover trigger threshold)
    - att_rx (T310Timer - Connection failure detection timer)
    - Others (Non-parameter queries)
    
    USER QUERY: {user_query}

    RULES:
    1. If the user asks for the current value of a parameter from the list:
       Use the enhanced tool find_value_in_gnb_enhanced(parameter_name) to retrieve the value from live network or simulation.
       Provide the retrieved value in your response with source information (live/simulation).
       Set Parameter: Others.

    2. If the user asks for the optimal value or best setting of a parameter from the list:
       Return the corresponding Parameter, such as "Parameter: p0_nominal", "Parameter: att_tx", etc.
       This will trigger full analysis and recommendations.

    3. If the query mentions network performance issues that could relate to these parameters:
       Identify the most relevant parameter based on the issue described.
       Example: "poor call setup" → p0_nominal, "handover failures" → att_tx

    4. If the query is unrelated to these network parameters:
       Respond politely that you can only handle network parameter optimization queries.
       Set Parameter: Others.

    CONTEXT: This is for Liquid Zimbabwe's live Huawei network with Bindura cluster data available.

    OUTPUT FORMAT:
    Reason:
    <Step-by-step reasoning here, mentioning live network context if relevant>
    ...
    # Get network mode status for context
    mode_status = get_network_mode_status()
    yield f"🌐 Configuration Mode: {mode_status.split()[0]} {mode_status.split()[1]}"
