"""
Restored Original Agent Implementation with Minimal Liquid Zimbabwe Integration
Based on the working template from bubbleran/Telco-Network-Configuration

This file restores the original 3-agent functionality:
...
# Import Liquid Zimbabwe monitoring system
try:
    from liquid_zimbabwe_monitoring_agent import liquid_zimbabwe_monitoring_agent
    LIQUID_ZIMBABWE_AVAILABLE = True
    print("✅ Liquid Zimbabwe monitoring system available")
except ImportError:
    print("⚠️ Liquid Zimbabwe monitoring not available - using BubbleRAN monitoring")
    LIQUID_ZIMBABWE_AVAILABLE = False
...
def monitoring_agent(state: State) -> State:
    """
    Enhanced Monitoring Agent with Liquid Zimbabwe Support
    
    This agent can use either:
    1. Liquid Zimbabwe real network monitoring (recommended) 
    2. Original BubbleRAN parameter monitoring (fallback)
    
    The choice is automatic based on data availability.
    """
    
    # Check if we should use Liquid Zimbabwe monitoring
    config = yaml.safe_load(open('config.yaml', 'r'))
    use_liquid_zimbabwe = (
        LIQUID_ZIMBABWE_AVAILABLE and 
        config.get('table_name') == 'kpi_data'  # Indicates we're using real network data
    )
    
    if use_liquid_zimbabwe:
        print("🇿🇼 Using Liquid Zimbabwe real network monitoring")
        try:
            return liquid_zimbabwe_monitoring_agent(state)
        except Exception as e:
            print(f"❌ Liquid Zimbabwe monitoring failed: {e}")
            print("🔄 Falling back to BubbleRAN monitoring")
            # Fall through to original monitoring
    else:
        print("🧪 Using BubbleRAN simulation monitoring")
    
    # Original BubbleRAN monitoring logic (unchanged)
    return monitoring_agent_original(state)


def monitoring_agent_original(state: State) -> State:
    """
