...
class AgenticDatabase:
    ...
    def _init_default_data(self, cursor):
        ...
        if cursor.fetchone()[0] == 0:
            ...
            default_agents = [
                ("Network Connector", "active", 3, {"capabilities": ["API connection", "data retrieval", "network status"]}),
                ("Monitoring Agent", "active", 1, {"capabilities": ["KPI monitoring", "threshold checking", "alert generation"]}),
                ("KPI Analytics", "active", 2, {"capabilities": ["trend analysis", "anomaly detection", "performance assessment"]}),
                ("Configuration Agent", "standby", 0, {"capabilities": ["parameter optimization", "configuration management", "automated tuning"]}),
                ("Validation Agent", "standby", 0, {"capabilities": ["result validation", "integrity checking", "rollback protection"]}),
                ("Execution Agent", "standby", 0, {"capabilities": ["command execution", "configuration deployment", "real-time monitoring"]})
            ]
    ...
    def get_active_agents(self) -> List[Dict]:
        """Get all active agents with status"""
        try:
            with sqlite3.connect(self.db_path) as conn:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor.fetchall():
                    op = dict(row)
                    if op['started_at']:
                        op['started_at'] = datetime.fromisoformat(op['started_at'])
                    if op['completed_at']:
