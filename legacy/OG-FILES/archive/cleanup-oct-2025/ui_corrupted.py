#!/usr/bin/env python3
"""
Liquid Zimbabwe 4G Network Optimizer - Production UI
Streamlit-based monitoring interface for live network data
"""
...
def get_live_kpi_data():
    ...
    try:
        ...
        return {
            ...
            "network_access_success_rate": round(database_accuracy, 1),
            "operational_sites": list(sites.keys()) if sites else []
        }
...
def get_network_parameters():
    ...
    try:
        ...
        sites = get_live_active_sites()
        if not sites:
            raise Exception("No live sites available")
...
def render_parameter_query_interface():
    ...
    with query_col3:
        ...
        if auto_refresh:
            ...
            # Trigger auto-refresh using st.rerun with timer
    ...
    # Auto-refresh logic
    if auto_refresh and st.session_state.get('auto_refresh_active', False):
        import time
    ...
    if 'query_result' in st.session_state:
        ...
        if result['status'] == 'success':
            ...
            st.error(f"❌ Query failed for {result['parameter']} from {result['site']}")
...
def main():
    ...
    # Header with Logo
    display_header_logo()
    st.markdown("**Network Monitoring and Configuration Dashboard**")
    st.markdown("---")
    ...
    with tab1:
        ...
        with col1:
            ...
            with kpi_col1:
                ...
                st.metric(
                    "Network Accuracy",
                    f"{kpi_data.get('database_accuracy', kpi_data.get('network_access_success_rate', 0))}%",
                    delta="0.2%"
                )
    ...
    st.markdown(f"🕐 Last updated: {current_time} | 🌍 Liquid Zimbabwe 4G Network | Phase 2 Ready")
