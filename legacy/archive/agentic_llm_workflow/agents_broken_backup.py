...
# Optional: Enhanced tools for live network integration
try:
    from agentic_llm_workflow.lz_api_client import get_api_client
    from agentic_llm_workflow.lz_config import is_liquid_zimbabwe_enabled, load_enhanced_config
    LZ_INTEGRATION_AVAILABLE = True
    print("🔹 Liquid Zimbabwe integration loaded")
except ImportError as e:
    LZ_INTEGRATION_AVAILABLE = False
    print(f"🔸 Liquid Zimbabwe integration not available: {e}")
...
def monitoring_agent(state: State) -> State:
    ...
    if LZ_INTEGRATION_AVAILABLE and is_liquid_zimbabwe_enabled():
        # Add status reporting for enhanced mode
        try:
            api_client = get_api_client()
            status = api_client.get_connection_status()
            yield f"📊 Network Status: {status}"
            ...
            if api_client.is_connected():
                try:
                    ...
                    yield f"🌐 Live KPIs: {kpi_summary}"
                except Exception as e:
                    yield f"⚠️ Live KPI fetch warning: {str(e)[:50]}..."
        except Exception as e:
            yield f"🔸 Enhanced mode initialization warning: {str(e)[:50]}..."
