"""
This file provides a collection of utilities to manage the live Huawei network environment,
including network lifecycle control (connect, disconnect, status checks), parameter management,
KPI data collection, and database synchronization between live network and local storage.
It supports real-time network parameter modifications and KPI analysis.
"""
...
try:
    ...
    try:
        ...
        print("Warning: Live network manager not available, falling back to simulation mode")

def check_network_status(print_output=False):
    """
    Checks if the live network is accessible or if simulation services are running.
    ...
    if LIVE_NETWORK_AVAILABLE:
        # Use live network status check
        return live_network_manager.check_network_status(print_output)
    
    # Fallback to original Docker-based check

    # Load network setup path from config.yaml
    # Load network setup path from configuration file
    # Get the project root directory (parent of agentic_llm_workflow)
    project_root = os.path.dirname(os.path.dirname(__file__))
    config_path = os.path.join(project_root, 'config.yaml')
    
    with open(config_path, 'r') as config_file:
        bubbleran_network_setup = yaml.safe_load(config_file)['bubbleran_network_setup']
    
    # In Docker, the paths are mounted to /workspace, so use that
    # The BUBBLERAN_HOST_PWD is for the Docker daemon access, not for paths inside container  
    docker_dir = os.path.join(project_root, bubbleran_network_setup)
    
    docker_file = os.path.join(docker_dir, "docker-compose.yaml")
    try:
...
def start_network():
    """
    Connects to the live Huawei network or starts Docker-based simulation as fallback.
    Yields:
        str: Status messages during the connection process.
    """
    
    if LIVE_NETWORK_AVAILABLE:
        # Use live network connection
        yield from live_network_manager.start_network()
        return
    
    # Fallback to original Docker-based startup
    ...
    # Use container paths for file operations, but host paths for Docker operations
    docker_dir = os.path.join(project_root, bubbleran_network_setup)
    
    # For Docker-in-Docker, we need to use the host path for docker compose
    # Get the host path from environment variable set in docker-compose.yaml
    bubbleran_host_pwd = os.environ.get('BUBBLERAN_HOST_PWD', project_root)
    host_docker_dir = os.path.join(bubbleran_host_pwd, bubbleran_network_setup)
    
    # For Docker-in-Docker, use a simpler approach - run docker compose from container
    # but let Docker resolve paths relative to the mounted volumes
    ...
    try:
        ...
        # Debug: Check if config files exist in container
        config_files_exist = []
        mx_conf_dir = os.path.join(docker_dir, 'mx-conf')
        for config_file in ['ric.yaml', 'xapp.yaml', 'e2_agent.yaml']:
            file_path = os.path.join(mx_conf_dir, config_file)
            exists = os.path.exists(file_path)
            config_files_exist.append((config_file, exists))
            print(f"Config file {config_file}: {'exists' if exists else 'missing'} at {file_path}")
        
        # Use host directory for docker compose to resolve bind mounts correctly
        subprocess.run(docker_run_command, cwd=host_docker_dir, check=True)
        

        # Check if additional UE connection setup is needed
        if "5g-sa-nr-sim" in bubbleran_network_setup:
...
def stop_network(reset_db = False):
    """
    Disconnects from live network or stops Docker-based simulation.
    ...
    if LIVE_NETWORK_AVAILABLE:
        # Use live network disconnection
        yield from live_network_manager.stop_network(reset_db)
        return
    
    # Fallback to original Docker-based stop
    # Load network setup path from configuration file
    # Get the project root directory (parent of agentic_llm_workflow)
    project_root = os.path.dirname(os.path.dirname(__file__))
    config_path = os.path.join(project_root, 'config.yaml')
    ...
    # Use container paths - the volumes are already mounted correctly in docker-compose.yaml
    docker_dir = os.path.join(project_root, bubbleran_network_setup)
    ...
    try:
        ...
        subprocess.run(docker_stop_command, cwd=host_docker_dir, check=True)
        time.sleep(4)
    except subprocess.CalledProcessError as e:
...
def add_traffic(traffic_bandwidth=100, traffic_time=15):
    ...
    if "5g-sa-nr-sim" not in bubbleran_network_setup:
