...
- Optimizer shape:
  - `/api/optimize` runs the AI workflow when LLM dependencies and keys are available.
  - If the LLM workflow is unavailable, a deterministic safe-mode optimizer still returns KPI evidence, recommendations, risk, and dry-run MML.
  - `/api/optimize/execute` remains dry-run unless explicitly approved by request and `NETGENIX_ALLOW_LIVE_MML=true`.
